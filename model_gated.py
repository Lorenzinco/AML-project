...
def sample_ellipses_mask(resolution:tuple[int, int], count_range: tuple[int, int], device="cpu"):
    ...
    ax = torch.empty(count, device=device).uniform_(0.1, 0.35)
    ay = torch.empty(count, device=device).uniform_(0.1, 0.35)
...
def mask_batch(batch: torch.Tensor, config:Config, device: torch.device):
    ...
    masked = (ell*(batch)) + torch.randn_like(batch) * (1-ell)
...
class GatedConv(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.activation(feature) * gate
